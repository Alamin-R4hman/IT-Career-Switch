...
//Another possible way to access the info is by creating a variable:

let space = spaceship.homePlanet;
...
/*
The selected code defines a function returnAnyProp that takes two arguments: objectName and propName. This function returns the property value from the objectName based on the propName given. In the example, returnAnyProp(spaceship, 'homePlanet') returns ‘Earth’.
...
/*
Once we’ve defined an object, we’re not stuck with all the properties we wrote. are mutable meaning we can update them after we create them!
...
In this lesson, the concept of “pass by reference” is explained using JavaScript objects. When an object is passed to a function, the function can directly change the object’s properties, and those changes will be permanent. This is because the function has a reference to the object’s memory location.

However, if a function tries to reassign the entire object to something new, it won’t affect the original object outside the function. The reassignment only changes the local reference inside the function, not the original object.
*/
...
for (let crewDetails in spaceship.crew) {
  console.log(`${property}: ${spaceship.crew[crewMemebr].name}`);
}