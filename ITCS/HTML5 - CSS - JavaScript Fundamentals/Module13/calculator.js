function equivalentCheck(){ // Allows last value to reamin in view until other buttons are pressed
...
function input(x){
  ...
  if(document.getElementById('decimalVar').value == 0){
    ...
    let deciamlCount = parseInt(document.getElementById('decimalVar').value);
    ...
    deciamlCount++;
    document.getElementById('decimalVar').value = deciamlCount;
...
function decimalPoint(){
  ...
  if(parseInt(document.getElementById('operation').value)) { //If this is an empty string it will return zero
    document.getElementById('result').value = 0;
  }
}
